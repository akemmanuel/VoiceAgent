...
describe("built extension", () => {
  test("uses MV3 with persistent browser access and strict privileged-page CSP", () => {
    ...
    expect(manifest.permissions ?? []).toEqual(["tabs", "scripting", "storage", "offscreen", "downloads", "sidePanel"]);
    expect(manifest.host_permissions ?? []).toEqual(["<all_urls>"]);
    expect(manifest.side_panel).toEqual({ default_path: "popup/index.html" });
